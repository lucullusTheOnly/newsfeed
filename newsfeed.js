...
Module.register("newsfeed",{
    ...
    document.getElementsByClassName("region bottom bar")[0].style.left = "0";
		if(!timer){
			this.scheduleUpdateInterval();
		}
	},
    ...
    } else if (notification === "ARTICLE_SCROLL_DOWN"){
      if(this.config.showFullArticle){
        this.scrollPosition += this.config.scrollLength;
        window.scrollTo(0, this.scrollPosition);
        Log.info(this.name + " - scrolling down");
	...
	showFullArticle: function() {
      ...
      document.getElementsByClassName("region bottom bar")[0].style.left = "-60px";
		}
		clearInterval(timer);
		timer = null;
